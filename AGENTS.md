...
Benchmark classes live under `src/test/java/com/tinyclaw/benchmark`. Run them from the IDE or a JMH launcher after `mvn test-compile`. A sample output file exists at `benchmark-results.txt`.
...
- `benchmark-results.txt` — untracked JMH output.
- `cp.txt` — untracked classpath dump, likely generated by an IDE.
